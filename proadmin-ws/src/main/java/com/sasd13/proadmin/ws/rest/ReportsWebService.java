...
public class ReportsWebService extends HttpServlet {
	...
	IReadService<Report> readService;
	IManageService<Report> manageService;
	...
	public void init() throws ServletException {
		...
		try {
			readService = ReadServiceFactory.make(Report.class);
			manageService = ManageServiceFactory.make(Report.class);
		} catch (WSException e) {
			LOG.error(e);
		}
	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		List<Report> reportsToResponse = new ArrayList<>();
		...
		try {
			...
			if (!parameters.isEmpty()) {
				reportsToResponse.addAll(readService.read(parameters));
			} else {
				reportsToResponse.addAll(readService.readAll());
			}

			resp.setContentType(RESPONSE_CONTENT_TYPE);
			Stream.writeAndClose(resp.getWriter(), ParserFactory.make(RESPONSE_CONTENT_TYPE).toString(reportsToResponse));
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			Report report = ParserFactory.make(req.getContentType()).fromString(Stream.readAndClose(req.getReader()), Report.class);
